/*
 * @Author: wk 
 * @Date: 2020-03-09 14:37:10 
 * @Last Modified by: wk
 * @Last Modified time: 2020-05-15 17:25:03
 * @Description:  查看数据
 */
<template>
  <div class="view-data"
        ...
        <div class="my-search">
          <div class="my-search-right">
            ...
            <div class="base-row">
              ...
              <!-- 表格 -->
              <el-table v-if="tableToggle&&headData.length>0"
                        :data="tableData"
                        stripe
                        fit
                        border
...
export default {
  ...
  methods: {
    ...
    exportXls() {
      ...
      basicDownload('/dataCheck/exportCheckData', exParans).then(_ => {
    ...
    getTableHeight() {
      this.$nextTick(_ => {
        ...
        const rightDom = document.querySelector('.aaab')
    ...
    // 获取配置查询条件
    getConfigData(cjrq) {
      baseRequest('/dataCheck/getConditions', { tableCode: this.tableCode, cjrq }).then(response => {
    ...
    searchOption(pageNoChange) {
      ...
      baseSearch('/dataCheck/getCheckData', params).then(response => {
