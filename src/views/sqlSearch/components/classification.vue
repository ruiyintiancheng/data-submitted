 ...
 * @Last Modified by: lk
 * @Last Modified time: 2019-09-23 13:59:50
 * @Description: 分类维护  
 */
<template>
...
export default {
  ...
  methods: {
    ...
    saveCla(row) {
      ...
      baseRequest(row.isAdd ? '/sqlGroup/add' : '/sqlGroup/update', row).then(_ => {
    ...
    deleteCla(row) {
      this.$confirm('确定删除该分类吗, 是否继续?', '提示', {
        ...
        baseRequest('/sqlGroup/delete', { groupId: row.groupId }).then(_ => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
