 ...
 * @Last Modified by: wk
 * @Last Modified time: 2020-05-15 17:02:16
 * @Description:  查看数据
 */
<template>
  <div class="view-data">
...
export default {
  ...
  methods: {
    ...
    exportXls() {
      ...
      basicDownload('/statisticxAnalysis/exportCheckData', exParans).then(_ => {
    ...
    getTableHeight() {
      this.$nextTick(_ => {
        ...
        this.tableHeight = lodheight - queryHeight - 150
      })
    },
    openDataView(row) {
    ...
    // 获取配置查询条件
    getConfigData() {
      baseRequest('/statisticxAnalysis/getConditions', { tableCode: this.tableCode }).then(response => {
